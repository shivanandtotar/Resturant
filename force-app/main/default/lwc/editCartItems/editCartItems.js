...
export default class EditCartItems extends LightningElement {
    ...
    handleModalBox() {
        ...
        if (this.openModal) {
            ...
            getOrderDetails({'orderId': this.recordId}).then(result => {
                console.log('line12 ', result);
                this.isOrderEditable = result.Status != 'Draft' ? false : true;
    ...
    handleOrder() {
        ...
        placeOrder({ 'orderItems': orderItemRecords, 'orderId': this.recordId })
            .then((result) => {
    ...
    handleAddProduct(event) {
        let [product] = this.products.filter((item) => item.Id == event.target.name)
        let updatedProd = { ...product, "quantity": 1 };
        console.log('line17 ', updatedProd)
        let tempProducts = this.products.filter((item) => item.Id != event.target.name)
    ...
    handleRemoveProduct(event) {
        let [product] = this.selectedProducts.filter((item) => item.Id == event.target.name);
        this.products.push(product)
        let tempProducts = this.selectedProducts.filter((item) => item.Id != event.target.name);
    ...
    handleSubCount(event) {
        this.selectedProducts.forEach(item => {
            if (item.Id == event.target.name) {
                if (item.quantity - 1 == 0) this.popProduct(item.Id)
    ...
    popProduct(id) {
        let [product] = this.selectedProducts.filter((item) => item.Id == id);
        this.products.push(product)
        let tempProducts = this.selectedProducts.filter((item) => item.Id != id);
        this.selectedProducts = [...tempProducts];
        this.calculateTotal();
    }
